import React from "react";

const page = () => {
  return (
    <div className="bg-white">
      <img
        src="https://www.parallelwireless.com/wp-content/uploads/blog-hero.jpg"
        alt=""
        className="w-screen h-74"
      />
      <h1 className="absolute top-0 left-0 right-0 bottom-0 flex items-center justify-center text-6xl text-black bg-gray-100 opacity-70 h-20 mt-[394px]">
        <span className="text-black">Solution Overviews</span>
      </h1>

      <div className="flex flex-row gap-12 justify-center  w-screen bg-gray-200 ">
        <a
          href="/white_papers"
          className="hover:bg-orange-500 hover:text-white text-black h-14 text-sm  text-center flex justify-center items-center"
        >
          WHITE PAPERS
        </a>
        <a
          href="/solution_overview"
          className="hover:bg-orange-500 hover:text-white text-black h-14 text-sm text-center flex justify-center items-center"
        >
          SOLUTION OVERVIEW
        </a>
        <a
          href="/videos"
          className="hover:bg-orange-500 hover:text-white text-black h-14 text-sm  text-center flex justify-center items-center"
        >
          VIDEOS
        </a>
        <a
          href="/blog"
          className="hover:bg-orange-500 hover:text-white text-black h-14 text-sm w-24  text-center flex justify-center items-center"
        >
          BLOG
        </a>
        <a
          href="/case_studies"
          className="hover:bg-orange-500 hover:text-white text-black h-14 text-sm  text-center flex justify-center items-center"
        >
          CASE STUDIES
        </a>
        <a
          href="/webinars"
          className="hover:bg-orange-500 hover:text-white text-black h-14 text-sm  text-center flex justify-center items-center"
        >
          WEBINARS
        </a>
        <a
          href="/open_ran"
          className="hover:bg-orange-500 hover:text-white text-black h-14 text-sm w-20 text-center flex justify-center items-center"
        >
          OPENRAN
        </a>
      </div>

      <div className="flex flex-wrap gap-14 justify-center flex-row p-10">
        {/* 1 */}
        <div className="">
          <div className="relative h-44 w-72 bg-gray-200 overflow-hidden border-b-8 border-orange-500">
            <div className="h-full transition-transform duration-500 ease-in-out transform hover:-translate-y-full">
              <h1 className="text-center text-black text-2xl flex justify-center items-center mt-8  p-6">
                Urban Macro Open RAN PDF
              </h1>
            </div>

            <div className="absolute inset-0 bg-orange-500 opacity-0 transition-all duration-500 ease-in-out hover:opacity-100">
              <h1 className="text-center text-white font-bold text-2xl flex justify-center items-center mt-12 p-6">
                <a href="" className="font-bold">
                  Download →
                </a>
              </h1>
            </div>
          </div>
        </div>
        {/* 2 */}
        <div className="">
          <div className="relative h-44 w-72 bg-gray-200 overflow-hidden border-b-8 border-orange-500">
            <div className="h-full transition-transform duration-500 ease-in-out transform hover:-translate-y-full">
              <h1 className="text-center text-black text-2xl flex justify-center items-center mt-8  p-6">
                2G Solution Overview PDF
              </h1>
            </div>

            <div className="absolute inset-0 bg-orange-500 opacity-0 transition-all duration-500 ease-in-out hover:opacity-100">
              <h1 className="text-center text-white font-bold text-2xl flex justify-center items-center mt-12 p-6">
                <a href="" className="font-bold">
                  Download →
                </a>
              </h1>
            </div>
          </div>
        </div>
        {/* 3 */}
        <div className="">
          <div className="relative h-44 w-72 bg-gray-200 overflow-hidden border-b-8 border-orange-500">
            <div className="h-full transition-transform duration-500 ease-in-out transform hover:-translate-y-full">
              <h1 className="text-center text-black text-2xl flex justify-center items-center mt-8  p-6">
                3G Solution Overview PDF
              </h1>
            </div>

            <div className="absolute inset-0 bg-orange-500 opacity-0 transition-all duration-500 ease-in-out hover:opacity-100">
              <h1 className="text-center text-white font-bold text-2xl flex justify-center items-center mt-12 p-6">
                <a href="" className="font-bold">
                  Download →
                </a>
              </h1>
            </div>
          </div>
        </div>
        {/* 4 */}
        <div className="">
          <div className="relative h-44 w-72 bg-gray-200 overflow-hidden border-b-8 border-orange-500">
            <div className="h-full transition-transform duration-500 ease-in-out transform hover:-translate-y-full">
              <h1 className="text-center text-black text-2xl flex justify-center items-center mt-8  p-6">
                4G Solution Overview PDF
              </h1>
            </div>

            <div className="absolute inset-0 bg-orange-500 opacity-0 transition-all duration-500 ease-in-out hover:opacity-100">
              <h1 className="text-center text-white font-bold text-2xl flex justify-center items-center mt-12 p-6">
                <a href="" className="font-bold">
                  Download →
                </a>
              </h1>
            </div>
          </div>
        </div>
        {/* 6 */}
        <div className="">
          <div className="relative h-44 w-72 bg-gray-200 overflow-hidden border-b-8 border-orange-500">
            <div className="h-full transition-transform duration-500 ease-in-out transform hover:-translate-y-full">
              <h1 className="text-center text-black text-2xl flex justify-center items-center mt-8  p-6">
                25 Solution Overview PDF
              </h1>
            </div>

            <div className="absolute inset-0 bg-orange-500 opacity-0 transition-all duration-500 ease-in-out hover:opacity-100">
              <h1 className="text-center text-white font-bold text-2xl flex justify-center items-center mt-12 p-6">
                <a href="" className="font-bold">
                  Download →
                </a>
              </h1>
            </div>
          </div>
        </div>
        {/* 7 */}
        <div className="">
          <div className="relative h-44 w-72 bg-gray-200 overflow-hidden border-b-8 border-orange-500">
            <div className="h-full transition-transform duration-500 ease-in-out transform hover:-translate-y-full">
              <h1 className="text-center text-black text-2xl flex justify-center items-center mt-8  p-6">
                Future 5G Dense Urban Deployments PDF
              </h1>
            </div>

            <div className="absolute inset-0 bg-orange-500 opacity-0 transition-all duration-500 ease-in-out hover:opacity-100">
              <h1 className="text-center text-white font-bold text-2xl flex justify-center items-center mt-12 p-6">
                <a href="" className="font-bold">
                  Download →
                </a>
              </h1>
            </div>
          </div>
        </div>
        {/* 8 */}
        <div className="">
          <div className="relative h-44 w-72 bg-gray-200 overflow-hidden border-b-8 border-orange-500">
            <div className="h-full transition-transform duration-500 ease-in-out transform hover:-translate-y-full">
              <h1 className="text-center text-black text-2xl flex justify-center items-center mt-8  p-6">
                OpenRAN Overview <br />
                PDF
              </h1>
            </div>

            <div className="absolute inset-0 bg-orange-500 opacity-0 transition-all duration-500 ease-in-out hover:opacity-100">
              <h1 className="text-center text-white font-bold text-2xl flex justify-center items-center mt-12 p-6">
                <a href="" className="font-bold">
                  Download →
                </a>
              </h1>
            </div>
          </div>
        </div>
        {/* 9 */}
        <div className="">
          <div className="relative h-44 w-72 bg-gray-200 overflow-hidden border-b-8 border-orange-500">
            <div className="h-full transition-transform duration-500 ease-in-out transform hover:-translate-y-full">
              <h1 className="text-center text-black text-2xl flex justify-center items-center mt-1  p-6">
                5G NR Logical <br />
                Architecture and its Functionality Splits
                <br />
                PDF
              </h1>
            </div>

            <div className="absolute inset-0 bg-orange-500 opacity-0 transition-all duration-500 ease-in-out hover:opacity-100">
              <h1 className="text-center text-white font-bold text-2xl flex justify-center items-center mt-12 p-6">
                <a href="" className="font-bold">
                  Download →
                </a>
              </h1>
            </div>
          </div>
        </div>
        {/* 10 */}
        <div className="">
          <div className="relative h-44 w-72 bg-gray-200 overflow-hidden border-b-8 border-orange-500">
            <div className="h-full transition-transform duration-500 ease-in-out transform hover:-translate-y-full">
              <h1 className="text-center text-black text-2xl flex justify-center items-center  p-6">
                OpenRAN Software Solution Overview PDF
              </h1>
            </div>

            <div className="absolute inset-0 bg-orange-500 opacity-0 transition-all duration-500 ease-in-out hover:opacity-100">
              <h1 className="text-center text-white font-bold text-2xl flex justify-center items-center mt-12 p-6">
                <a href="" className="font-bold">
                  Download →
                </a>
              </h1>
            </div>
          </div>
        </div>
        {/* 11 */}
        <div className="">
          <div className="relative h-44 w-72 bg-gray-200 overflow-hidden border-b-8 border-orange-500">
            <div className="h-full transition-transform duration-500 ease-in-out transform hover:-translate-y-full">
              <h1 className="text-center text-black text-2xl flex justify-center items-center  p-6">
                Tackling Network Densification Challenges for 5G PDF
              </h1>
            </div>

            <div className="absolute inset-0 bg-orange-500 opacity-0 transition-all duration-500 ease-in-out hover:opacity-100">
              <h1 className="text-center text-white font-bold text-2xl flex justify-center items-center mt-12 p-6">
                <a href="" className="font-bold">
                  Download →
                </a>
              </h1>
            </div>
          </div>
        </div>
        {/* 12 */}
        <div className="">
          <div className="relative h-44 w-72 bg-gray-200 overflow-hidden border-b-8 border-orange-500">
            <div className="h-full transition-transform duration-500 ease-in-out transform hover:-translate-y-full">
              <h1 className="text-center text-black text-2xl flex justify-center items-center  p-6">
                Tackling Network Densification Challenges for 5G PDF
              </h1>
            </div>

            <div className="absolute inset-0 bg-orange-500 opacity-0 transition-all duration-500 ease-in-out hover:opacity-100">
              <h1 className="text-center text-white font-bold text-2xl flex justify-center items-center mt-12 p-6">
                <a href="" className="font-bold">
                  Download →
                </a>
              </h1>
            </div>
          </div>
        </div>
        {/* 13 */}
        <div className="">
          <div className="relative h-44 w-72 bg-gray-200 overflow-hidden border-b-8 border-orange-500">
            <div className="h-full transition-transform duration-500 ease-in-out transform hover:-translate-y-full">
              <h1 className="text-center text-black text-2xl flex justify-center items-center mt-6 p-6">
                4G Densification
                <br />
                PDF
              </h1>
            </div>

            <div className="absolute inset-0 bg-orange-500 opacity-0 transition-all duration-500 ease-in-out hover:opacity-100">
              <h1 className="text-center text-white font-bold text-2xl flex justify-center items-center mt-12 p-6">
                <a href="" className="font-bold">
                  Download →
                </a>
              </h1>
            </div>
          </div>
        </div>
        {/* 14 */}
        <div className="">
          <div className="relative h-44 w-72 bg-gray-200 overflow-hidden border-b-8 border-orange-500">
            <div className="h-full transition-transform duration-500 ease-in-out transform hover:-translate-y-full">
              <h1 className="text-center text-black text-xl flex justify-center items-center  p-6">
                What Happens to Deployment TCO When Mobile Operators Deploy
                OpenRAN Only For 5G PDF
              </h1>
            </div>

            <div className="absolute inset-0 bg-orange-500 opacity-0 transition-all duration-500 ease-in-out hover:opacity-100">
              <h1 className="text-center text-white font-bold text-2xl flex justify-center items-center mt-12 p-6">
                <a href="" className="font-bold">
                  Download →
                </a>
              </h1>
            </div>
          </div>
        </div>

        {/* **************************** */}
      </div>
    </div>
  );
};

export default page;
